...
*-------------------- PARAMETERS ---------------------------------------
      ...
      real*8    delgrid,boxsize,rminex,tol
...
*-------------------- LOCAL VARIABLES ----------------------------------
      ...
      real*8    cgtotc,cgtotpx,cgtotpy,cgtotpz
...
c-----------------------------------------centre de masse de la proteine
      ...
      xmin=bx-boxsize/2.0D0
      ymin=by-boxsize/2.0D0
      zmin=bz-boxsize/2.0D0
        ...
        do ll=1,ngrid
          ...
          do mm=1,ngrid
            ...
            do ii=1,natom
              ...
              if (rr.lt.rminsol) rminsol=rr
              if (rr.le.radius(ii)) then
                solvent=.false.
                goto 2101
              endif
            enddo
            ...
            do jj=1,nbead
              ...
              rr=dsqrt(xx*xx+yy*yy+zz*zz)
              if (rr.lt.rminsol) rminsol=rr
              if (rr.le.cgrad(jj)) then
                solvent=.false.
                goto 2101
              endif
            enddo

            if (rminsol.gt.rminex) solvent=.false.
...
c------------------------------------difference de potentiel maximum
...
c-------------------------------------verifications et output
      ...
      do jj=1,nbead
        cgtotc=cgtotc+cgchg(jj)
        cgtotpx=cgtotpx+cgchg(jj)*(cgcox(jj)-bx)
        cgtotpy=cgtotpy+cgchg(jj)*(cgcoy(jj)-by)
